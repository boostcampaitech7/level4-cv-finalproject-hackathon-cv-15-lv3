...
class HNSWSearch:
    ...
    def _initialize_hnsw(self):
        self.index = hnswlib.Index(space='l2', dim=self.embeddings.shape[1])
        self.index.init_index(max_elements=len(self.embeddings), ef_construction=200, M=16)
        for i, emb in enumerate(self.embeddings):
            self.index.add_items(emb, i)
    ...
    def find_similar_captions(self, input_text, top_k=3):
        ...
        return [
            (self.translator.translate_en_to_ko(self.captions[i]), distances[0][idx], self.filtered_data[i])
