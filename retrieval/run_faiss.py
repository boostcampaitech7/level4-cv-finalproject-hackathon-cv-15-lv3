...
import faiss
import numpy as np
from search.faiss_search import FaissSearch  # FAISSSearch 클래스가 정의된 파일

def main():
    parser = argparse.ArgumentParser(description="FAISS 기반 비디오 캡션 검색")
    ...
    searcher = FaissSearch(json_path=args.json)
    
    count = 0
    sum_time = 0

    while True:
        query = input("FAISS 검색할 텍스트를 입력 (종료하려면 'exit' 입력): ")
        if query.lower() == "exit":
            print("프로그램 종료!")
            break
