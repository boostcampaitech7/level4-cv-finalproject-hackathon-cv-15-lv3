...
def save_segment(video_path, output_path, start_time, end_time):
    ...
    subprocess.run(command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # ffmpeg 실행
...
def process_videos_from_json(json_file, video_base_path, designated_path, start, end, segment_method="fixed", segment_duration=1):
    ...
    for video_info in video_metadata:
...
video_base_path = "/data/ephemeral/home/data/YouTube-8M-video"  # 원본 비디오가 저장된 폴더 경로
designated_path = "/data/ephemeral/home/jiwan/level4-cv-finalproject-hackathon-cv-15-lv3/pre-processing/YouTube-8M/data/videos"  # 클립이 저장될 폴더 경로
start_video = 1  # 처리할 비디오 시작 번호
end_video = 1 # 처리할 비디오 종료 번호

# 지정한 범위 내의 비디오를 처리하여 JSON 데이터 생성
output_json = process_videos_from_json(json_file, video_base_path, designated_path, start=start_video, end=end_video, segment_method="fixed", segment_duration=1)

# JSON 데이터를 파일로 저장
with open("/data/ephemeral/home/jiwan/level4-cv-finalproject-hackathon-cv-15-lv3/pre-processing/YouTube-8M/data/video_segments.json", "w", encoding="utf-8") as json_file:
