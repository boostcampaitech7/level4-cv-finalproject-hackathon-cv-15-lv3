...
def model_factory(train_config, model_config, **kwargs):
    ...
    #logger.info("llm Model dtype: {}".format(next(llm.parameters()).dtype))
    # projector
    ...
    if (train_config.enable_fsdp or train_config.enable_ddp) and train_config.low_cpu_fsdp:
        ...
        if rank == 0:
            if "vallex" in model_config.llm_name.lower():
                ...
                '''model = AutoAWQForCausalLM.from_quantized(model_config.llm_path, fuse_layers=True,
...
def setup_encoder_projector(train_config, model_config, **kwargs):
    ...
    '''if model_config.get("projector_ckpt_path", None):
        ckpt_path = model_config.projector_ckpt_path
        print(f"Loading projector weights from {ckpt_path}")
        state_dict = torch.load(ckpt_path, map_location="cpu")
        new_state_dict = {k.replace("encoder_projector.", ""): v for k, v in state_dict.items()}
        encoder_projector.load_state_dict(new_state_dict)'''
