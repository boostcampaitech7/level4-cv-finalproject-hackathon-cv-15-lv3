...
class AutoModelForSentenceEmbedding(nn.Module):
    ...
    def forward(self, **kwargs):
        model_output = self.model(**kwargs)  # Forward pass through transformer model
        embeddings = self.mean_pooling(model_output, kwargs['attention_mask'])  # Apply mean pooling
        if self.normalize:
            embeddings = torch.nn.functional.normalize(embeddings, p=2, dim=1)  # Apply L2 normalization
        return embeddings
...
def train_function(index, args, queue):
    ...
    lr_scheduler = get_linear_schedule_with_warmup(
        optimizer=optimizer, num_warmup_steps=500, num_training_steps=args.steps
    )
    ...
    for global_step in tqdm.trange(args.steps):
        ...
        if len(batch[0]) == 2:  # (anchor, positive)
            text1 = tokenizer([b[0] for b in batch], return_tensors="pt", max_length=args.max_length, truncation=True, padding="max_length").to(device)
            text2 = tokenizer([b[1] for b in batch], return_tensors="pt", max_length=args.max_length, truncation=True, padding="max_length").to(device)

            embeddings_a = model(**text1)
            embeddings_b = model(**text2)
            ...
            text1 = tokenizer([b[0] for b in batch], return_tensors="pt", max_length=args.max_length, truncation=True, padding="max_length").to(device)
            text2 = tokenizer([b[1] for b in batch], return_tensors="pt", max_length=args.max_length, truncation=True, padding="max_length").to(device)
            text3 = tokenizer([b[2] for b in batch], return_tensors="pt", max_length=args.max_length, truncation=True, padding="max_length").to(device)

            embeddings_a = model(**text1)
            embeddings_b1 = model(**text2)
            embeddings_b2 = model(**text3)
    ...
    print("save model final:", output_path)
    model.save_pretrained(output_path)
...
def produce_data(args, queue, filepaths, dataset_indices):
    ...
    print(size_per_dataset,"@@@@@@@@@@@")
    num_same_dataset = max(1, int(size_per_dataset / args.batch_size))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nprocs', type=int, default=torch.cuda.device_count())  # GPU 개수 자동 설정
    ...
    # Start producer (데이터 로딩 프로세스)
    p = mp.Process(target=produce_data, args=(args, queue, filepaths, dataset_indices))
    p.start()

    # Start training (멀티 GPU 지원)
    print(f"Using {args.nprocs} GPUs for training")
    processes = []
    for rank in range(args.nprocs):
        p = mp.Process(target=train_function, args=(rank, args, queue))
        p.start()
        processes.append(p)
